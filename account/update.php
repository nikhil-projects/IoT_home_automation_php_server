...
$stmt = $mysqli->prepare("UPDATE `account` SET `name`= ?,`user_name`= ?,`email`= ?,`phone`= ?  WHERE id = ? ");
$stmt->bind_param("ssssi", $data['name'] , $data['user_name'] , $data['email'] , $data['phone'], $user_id);
if(!$stmt->execute()){
